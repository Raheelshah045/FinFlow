...
export const AdminPage = ({ onLogout }) => {
    ...
    return (
        <div style={{
            height: "100vh",
            width: "100%",
            background: "radial-gradient(circle at top right, #1a4d2e, transparent), radial-gradient(circle at bottom left, #064e3b, transparent), #052e16",
            ...
            <div style={{ padding: "24px 40px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid rgba(255, 255, 255, 0.08)" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    <div style={{ background: "rgba(132, 204, 22, 0.2)", padding: 10, borderRadius: 14, border: "1px solid rgba(132, 204, 22, 0.3)" }}>
                        <Icon name="shield" size={24} color="#a3e635" />
                    </div>
                    <div>
                ...
                <div style={{ display: "flex", gap: 12 }}>
                    <button onClick={() => setShowCreateModal(true)} style={{ background: "#84cc16", color: "#064e3b", border: "none", padding: "12px 24px", borderRadius: 14, fontWeight: 700, cursor: "pointer", display: "flex", alignItems: "center", gap: 10, boxShadow: "0 10px 15px -3px rgba(132, 204, 22, 0.3)" }}>
                        ...
                        <tbody>
                            {users.map(u => (
                                <tr key={u.id}>
                                    ...
                                    <td style={{ fontFamily: "monospace", color: "#a3e635" }}>@{u.username}</td>
                                    <td>{u.email}</td>
                                    <td>{u.phone}</td>
                                    <td>
                                        <span style={{ padding: "4px 10px", borderRadius: 6, fontSize: 11, fontWeight: 800, background: u.role === 'admin' ? "rgba(132, 204, 22, 0.2)" : "rgba(148, 163, 184, 0.1)", color: u.role === 'admin' ? "#a3e635" : "#94a3b8" }}>
                                            {u.role.toUpperCase()}
                                        </span>
                                    </td>
                                    <td>
            ...
            {showCreateModal && (
                <div className="modal-overlay">
                    <div className="glass-card" style={{ width: "100%", maxWidth: 460, padding: 40, background: "#052e16" }}>
                        ...
                        <form onSubmit={handleCreateUser}>
                            ...
                            <button type="submit" disabled={loading} style={{ width: "100%", background: "linear-gradient(135deg, #84cc16, #65a30d)", color: "#064e3b", border: "none", padding: 16, borderRadius: 12, fontWeight: 700, cursor: "pointer", fontSize: 16 }}>
            ...
            {selectedUser && (
                <div className="modal-overlay">
                    <div className="glass-card" style={{ width: "100%", maxWidth: 420, padding: 40, background: "#052e16", textAlign: "center" }}>
                        <div style={{ width: 80, height: 80, background: "rgba(132, 204, 22, 0.1)", borderRadius: 30, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px" }}>
                            <Icon name="user" size={40} color="#84cc16" />
                        </div>
                        <h2 style={{ color: "#fff", fontSize: 22, fontWeight: 800, margin: "0 0 8px" }}>{selectedUser.name}</h2>
                        <p style={{ color: "#a3e635", fontFamily: "monospace", margin: "0 0 32px" }}>@{selectedUser.username}</p>
